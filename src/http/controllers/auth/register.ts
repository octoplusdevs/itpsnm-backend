...
export async function registerController(request: FastifyRequest, reply: FastifyReply) {
  ...
  const { email, password, role, employeeId, studentId } = registerSchema.parse(request.body);
  ...
  try {
    // Execução do UseCase
    const result = await registerUseCase.execute({ email, password, role, employeeId, studentId });
    if (result.success) {
      // Retornar sucesso
