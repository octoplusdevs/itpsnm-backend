...
// src/use-cases/ItemPaymentDetail/create-item-payment-detail.spec.ts
...
var InMemoryItemPaymentDetailsRepository = class {
  ...
  async destroy(itemPaymentDetailId) {
    const index2 = this.itemPaymentDetails.findIndex((item) => item.id === itemPaymentDetailId);
    if (index2 !== -1) {
      this.itemPaymentDetails.splice(index2, 1);
      return true;
    }
    return false;
  }
...
var CreateItemPaymentDetailUseCase = class {
  ...
  async execute(request) {
    const { type: type2, quantity, price, paymentsId } = request;
...
describe("CreateItemPaymentDetailUseCase", () => {
  it("should create an item payment detail", async () => {
    ...
    const itemDetail = await createItemPaymentDetailUseCase.execute({
      type: import_client.PaymentType.A,
      quantity: 2,
      price: 50,
      paymentsId: 1
    });
    globalExpect(itemDetail).toBeDefined();
    globalExpect(itemDetail.type).toBe(import_client.PaymentType.A);
    globalExpect(itemDetail.quantity).toBe(2);
    globalExpect(itemDetail.price).toBe(50);
    globalExpect(itemDetail.paymentsId).toBe(1);
    globalExpect(itemDetail.created_at).toBeInstanceOf(Date);
    globalExpect(itemDetail.update_at).toBeInstanceOf(Date);
    const storedItemDetail = await itemPaymentDetailsRepository.findByPaymentId(itemDetail.paymentsId);
    globalExpect(storedItemDetail).toContainEqual(itemDetail);
  });
});
