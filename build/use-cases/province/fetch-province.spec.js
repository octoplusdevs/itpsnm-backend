...
var FetchProvinceUseCase = class {
  constructor(provincesRepository2) {
    this.provincesRepository = provincesRepository2;
  }
  async execute({
    name,
    page
...
// src/use-cases/province/fetch-province.spec.ts
...
describe("Fetch Province Use Case", () => {
  ...
  it("should be able to fetch a province", async () => {
    ...
    globalExpect(provinces).toHaveLength(1);
    globalExpect(provinces).toEqual([
      globalExpect.objectContaining({ name: "Benguela" })
    ]);
  });
  it("should be able to fetch paginated provinces", async () => {
    ...
    globalExpect(provinces).toHaveLength(2);
    globalExpect(provinces).toEqual([
      globalExpect.objectContaining({ name: "province-21" }),
      globalExpect.objectContaining({ name: "province-22" })
    ]);
  });
});
