...
describe("LoginUseCase", () => {
  ...
  test("should return success and token on successful login", async () => {
    ...
    globalExpect(response.success).toBe(true);
    globalExpect(response.message).toBe("Login successful");
    globalExpect(response.token).toBeDefined();
    const decoded = import_jsonwebtoken2.default.verify(response.token, jwtSecret);
    globalExpect(decoded).toHaveProperty("userId");
    globalExpect(decoded).toHaveProperty("role");
  });
  test("should block account after 3 failed login attempts", async () => {
    ...
    globalExpect(response.success).toBe(false);
    globalExpect(response.message).toBe("Account blocked due to multiple failed login attempts.");
    const user = await userRepository.findByEmail(email);
    globalExpect(user?.isBlocked).toBe(true);
  });
  test("should return error for blocked account", async () => {
    ...
    const response = await loginUseCase.execute({ email, password });
    globalExpect(response.success).toBe(false);
    globalExpect(response.message).toBe("Account is blocked. Please contact support.");
  });
  test("should return error for invalid credentials", async () => {
    ...
    await userRepository.create({
      email,
      password: await import_bcryptjs2.default.hash(password, 8),
      role: "STUDENT"
    });
    const response = await loginUseCase.execute({ email, password: "wrongpassword" });
    globalExpect(response.success).toBe(false);
    globalExpect(response.message).toBe("Invalid credentials.");
  });
  test("should reset login attempts on successful login", async () => {
    ...
    globalExpect(response.message).toBe("Login successful");
    const user = await userRepository.findByEmail(email);
    globalExpect(user?.loginAttempt).toBe(0);
  });
});
