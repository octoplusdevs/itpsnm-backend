...
// src/use-cases/authenticate/register.spec.ts
...
describe("RegisterUseCase", () => {
  ...
  test("should register a user successfully and return a token", async () => {
    ...
    globalExpect(response.message).toBe("User registered successfully");
  ...
  test("should not register a user with an existing email", async () => {
    ...
    globalExpect(response.message).toBe("Email already in use");
  });
  test("should hash the password before saving the user", async () => {
    ...
    globalExpect(user).toBeDefined();
    const isPasswordHashed = await import_bcryptjs2.default.compare(password, user.password);
  ...
  test("should return the registered user ID", async () => {
    ...
    const response = await registerUseCase.execute({ email, password, role: "STUDENT" });
    globalExpect(response.success).toBe(true);
    globalExpect(response.userId).toBeDefined();
    const user = await userRepository.findByEmail(email);
    globalExpect(user?.id).toBe(response.userId);
  });
});
