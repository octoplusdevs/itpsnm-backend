...
// src/use-cases/document/create-document-with-files.spec.ts
...
var InMemoryDocumentsRepository = class {
  ...
  async destroy(DocumentId) {
    const index2 = this.items.findIndex((item) => item.id === DocumentId);
    if (index2 !== -1) {
...
var InMemoryLevelsRepository = class {
  ...
  async create(data) {
    const level = {
      id: data.id ?? (0, import_crypto3.randomInt)(99999),
      name: data.name,
      created_at: /* @__PURE__ */ new Date(),
      update_at: /* @__PURE__ */ new Date()
    };
...
describe("Create Document With Files Use Case", () => {
  ...
  let createDocumentWithFilesUseCase;
      ...
      files: [
        {
          ...
          format: import_client.FileFormat.PDF,
          type: import_client.FileType.REPORT_CARD,
        ...
        },
        {
          ...
          format: import_client.FileFormat.PDF,
          type: import_client.FileType.IDENTITY_CARD,
          identityCardNumber: "1"
        }
      ]
    });
    globalExpect(document).toHaveProperty("id");
    globalExpect(files.length).toBe(2);
    globalExpect(files[0].documentId).toEqual(document.id);
    globalExpect(files[1].documentId).toEqual(document.id);
  });
});
