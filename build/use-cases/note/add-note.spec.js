...
var CreateNoteUseCase = class {
  constructor(notesRepository2, enrollmentsRepository2, subjectsRepository2) {
    this.notesRepository = notesRepository2;
    this.enrollmentsRepository = enrollmentsRepository2;
    this.subjectsRepository = subjectsRepository2;
  }
  async execute({
    pf1,
    pf2,
...
var InMemoryNotesRepository = class {
  ...
  async addNote(data) {
    const note = {
      id: data.id ?? (0, import_crypto2.randomInt)(99999),
      pf1: data.pf1 ?? 0,
      pf2: data.pf2 ?? 0,
      pft: data.pft ?? 0,
      nee: data.nee ?? 0,
  ...
  async destroy(id) {
    const index2 = this.items.findIndex((item) => item.id === id);
    if (index2 !== -1) {
...
var InMemoryEnrollmentRepository = class {
  ...
  async destroy(enrollmentId) {
    const index2 = this.items.findIndex((item) => item.id === enrollmentId);
    if (index2 !== -1) {
      this.items.splice(index2, 1);
      return true;
    }
  ...
  async create(data) {
    const enrollment = {
      id: data.id ?? (0, import_crypto3.randomInt)(9999),
...
var InMemorySubjectsRepository = class {
  ...
  async create(data) {
    const course = {
      id: data.id ?? (0, import_crypto4.randomInt)(99999),
      name: data.name,
      created_at: /* @__PURE__ */ new Date(),
      update_at: /* @__PURE__ */ new Date()
    };
...
// src/use-cases/note/add-note.spec.ts
var notesRepository;
...
describe("Create Note Use Case", () => {
  ...
  it("should create a new note", async () => {
    ...
    globalExpect(response.note).toHaveProperty("id");
    globalExpect(response.note.pf1).toEqual(15);
    globalExpect(response.note.pf2).toEqual(17);
    globalExpect(response.note.pft).toEqual(20);
    globalExpect(response.note.nee).toEqual(5);
    globalExpect(response.note.resource).toEqual(10);
    globalExpect(response.note.enrollmentId).toEqual(123);
    globalExpect(response.note.subjectId).toEqual(1);
    globalExpect(response.note.created_at).toBeInstanceOf(Date);
    globalExpect(response.note.update_at).toBeInstanceOf(Date);
  });
  it("should assign default values when fields are not provided", async () => {
    ...
    globalExpect(response.note.pf1).toEqual(0);
    globalExpect(response.note.pf2).toEqual(0);
    globalExpect(response.note.pft).toEqual(0);
    globalExpect(response.note.nee).toEqual(0);
    globalExpect(response.note.resource).toEqual(0);
    globalExpect(response.note.enrollmentId).toEqual(1);
    globalExpect(response.note.subjectId).toEqual(1);
  });
  it("should save the note in the repository", async () => {
    ...
    globalExpect(notes[0].enrollmentId).toEqual(456);
    globalExpect(notes[0].subjectId).toEqual(1);
  });
});
