...
var CreateEnrollmentUseCase = class {
  constructor(levelsRepository2, coursesRepository2, enrollmentRepository, studentRepository) {
    this.levelsRepository = levelsRepository2;
    this.coursesRepository = coursesRepository2;
...
// src/repositories/in-memory/in-memory-courses-repository.ts
var import_crypto4 = require("crypto");
var InMemoryCoursesRepository = class {
  constructor() {
    this.items = [];
  }
  async findById(id) {
...
var InMemoryLevelsRepository = class {
  ...
  async create(data) {
    const level = {
      id: data.id ?? (0, import_crypto5.randomInt)(99999),
      name: data.name,
      created_at: /* @__PURE__ */ new Date(),
      update_at: /* @__PURE__ */ new Date()
    };
...
// src/use-cases/enrollment/create-enrollment.spec.ts
...
describe("Create Enrollment Use Case", () => {
  ...
  it("should be able to create an enrollment", async () => {
    const student = await studentsRepository.create({
      id: 1,
      fullName: "John Doe",
      dateOfBirth: /* @__PURE__ */ new Date("2000-01-01"),
    ...
    const { enrollment } = await sut.execute({
      id: 1,
      docsState: import_client.EnrollementState.PENDING,
      paymentState: import_client.EnrollementState.PENDING,
      identityCardNumber: "1234567890",
      courseId: course.id,
      levelId: level.id,
      created_at: /* @__PURE__ */ new Date(),
    ...
    globalExpect(enrollment.identityCardNumber).toBe("1234567890");
    globalExpect(enrollment.docsState).toBe(import_client.EnrollementState.PENDING);
    globalExpect(enrollment.paymentState).toBe(import_client.EnrollementState.PENDING);
  });
