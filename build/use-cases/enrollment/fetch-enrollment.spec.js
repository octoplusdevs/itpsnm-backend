...
// src/use-cases/enrollment/fetch-enrollment.spec.ts
...
describe("Fetch Enrollments Use Case", () => {
  ...
  it("should be able to fetch all enrollments", async () => {
    ...
    globalExpect(enrollments?.items).toHaveLength(1);
    globalExpect(enrollments?.items).toEqual([
      globalExpect.objectContaining({ docsState: "APPROVED", paymentState: "APPROVED" })
    ]);
  });
  it("should be able to fetch paginated enrollment", async () => {
    ...
    globalExpect(enrollments?.items).toHaveLength(2);
    globalExpect(enrollments?.items).toEqual([
      globalExpect.objectContaining({ docsState: "REJECTED" }),
      globalExpect.objectContaining({ paymentState: "REJECTED" })
    ]);
  });
});
