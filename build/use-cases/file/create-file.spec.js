...
// src/use-cases/file/create-file.spec.ts
...
var CreateFileUseCase = class {
  ...
  async execute({ name, path: path2, format: format4, type: type2, identityCardNumber, documentId }) {
...
describe("CreateFileUseCase", () => {
  ...
  it("should create a new file", async () => {
    const file = await createFileUseCase.execute({
      ...
      type: import_client.FileType.TUITION_RECEIPT,
      studentId: 1
    });
    globalExpect(file).toHaveProperty("id");
    globalExpect(file.name).toBe("test-file");
  });
});
