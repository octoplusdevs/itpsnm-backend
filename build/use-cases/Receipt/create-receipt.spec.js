...
var InMemoryReceiptsRepository = class {
  ...
  async destroy(receiptId) {
    const index2 = this.receipts.findIndex((receipt) => receipt.id === receiptId);
    if (index2 !== -1) {
      this.receipts.splice(index2, 1);
      return true;
    }
    return false;
  }
...
var CreateReceiptUseCase = class {
  ...
  async execute(request) {
    const { tuition_id, path: path2, payment_id } = request;
...
// src/use-cases/Receipt/create-receipt.spec.ts
describe("CreateReceiptUseCase", () => {
  it("should create a receipt", async () => {
    ...
    globalExpect(receipt).toBeDefined();
    globalExpect(receipt.tuition_id).toBe(1);
    globalExpect(receipt.path).toBe("/path/to/receipt.pdf");
    globalExpect(receipt.payment_id).toBe(1);
    globalExpect(receipt.created_at).toBeInstanceOf(Date);
    globalExpect(receipt.update_at).toBeInstanceOf(Date);
    const storedReceipt = await receiptsRepository.findById(receipt.id);
    globalExpect(storedReceipt).toEqual(receipt);
  });
});
