...
// src/use-cases/payment/create-payment.spec.ts
...
var InMemoryPaymentsRepository = class {
  ...
  async destroy(paymentId) {
    const index2 = this.payments.findIndex((payment) => payment.id === paymentId);
    if (index2 !== -1) {
      this.payments.splice(index2, 1);
      return true;
    }
    return false;
  }
...
var InMemoryStudentRepository = class {
  ...
  async create(data) {
    const newStudent = {
      id: data.id ?? (0, import_crypto2.randomInt)(9999),
      type: data.type,
...
describe("CreatePaymentUseCase", () => {
  it("should create a payment with multiple items", async () => {
    ...
    await studentsRepository.create({
      ...
      dateOfBirth: /* @__PURE__ */ new Date("01/08/2000"),
      email: "daniel.yava16@gmail.com",
      emissionDate: /* @__PURE__ */ new Date(),
    ...
    const payment = await createPaymentUseCase.execute({
      identityCardNumber: "123456789",
      state: import_client.PaymentState.PENDING,
      date: /* @__PURE__ */ new Date(),
      items: [
        { type: import_client.PaymentType.A, quantity: 2, price: 50 },
        { type: import_client.PaymentType.B, quantity: 1, price: 100 }
      ]
    });
    globalExpect(payment).toBeDefined();
    globalExpect(payment.identityCardNumber).toBe("123456789");
    globalExpect(payment.state).toBe(import_client.PaymentState.PENDING);
    globalExpect(payment.amount_paid).toBe(200);
    globalExpect(payment.date).toBeInstanceOf(Date);
    globalExpect(payment.created_at).toBeInstanceOf(Date);
    globalExpect(payment.update_at).toBeInstanceOf(Date);
    const storedPayment = await paymentsRepository.findById(payment.id);
    globalExpect(storedPayment).toEqual(payment);
    const items = paymentsRepository.itemPaymentDetails.filter((item) => item.paymentsId === payment.id);
    globalExpect(items.length).toBe(2);
    globalExpect(items).toEqual(globalExpect.arrayContaining([
      globalExpect.objectContaining({ type: import_client.PaymentType.A, quantity: 2, price: 50 }),
      globalExpect.objectContaining({ type: import_client.PaymentType.B, quantity: 1, price: 100 })
    ]));
  });
});
