...
// src/use-cases/student/fetch-course.spec.ts
...
describe("Fetch Courses Use Case", () => {
  ...
  it("should be able to fetch a course", async () => {
    ...
    globalExpect(courses).toHaveLength(1);
    globalExpect(courses).toEqual([
      globalExpect.objectContaining({ name: "curso 01" })
    ]);
  });
  it("should be able to fetch paginated course", async () => {
    ...
    globalExpect(courses).toHaveLength(2);
    globalExpect(courses).toEqual([
      globalExpect.objectContaining({ name: "course-21" }),
      globalExpect.objectContaining({ name: "course-22" })
    ]);
  });
});
